...
public class Vars implements Loadable {
    ...
    /**
     * Whether to enable various experimental features (e.g. spawn positions for
     * spawn groups) TODO change
     */
    ...
    public static String serverJsonBeURL = "https://raw.githubusercontent.com/Anuken/Mindustry/master/servers_be.json";
    /** URL to the JSON file containing all the stable servers. */
    // TODO merge with v6 list upon release
    public static String serverJsonURL = "https://raw.githubusercontent.com/Anuken/Mindustry/master/servers_v7.json";
    ...
    public static void init() {
        ...
        javaPath = new Fi(OS.prop("java.home")).child("bin/java").exists()
                ? new Fi(OS.prop("java.home")).child("bin/java").absolutePath()
                : Core.files.local("jre/bin/java").exists() ? Core.files.local("jre/bin/java").absolutePath() : // Unix
                        Core.files.local("jre/bin/java.exe").exists()
                                ? Core.files.local("jre/bin/java.exe").absolutePath()
                                : // Windows
                                "java";
    ...
    public static void loadFileLogger() {
        ...
        try {
            ...
            Log.logger = (level, text) -> {
                ...
                try {
                    writer.write(
                            "[" + Character.toUpperCase(level.name().charAt(0)) + "] " + Log.removeColors(text) + "\n");
                    writer.flush();
                } catch (IOException e) {
                    e.printStackTrace();
                    // ignore it
